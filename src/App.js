  ...
  constructor() {
    ...
    this.state = {
      todos: [
        ...
        {key: '10', data:{ name: 'manger le salut de coucou', isDone: false}}
      ],
      topBar: '',
      option: 'All'
    }
  ...
  handleOnClick = (pIndex) =>
    this.setState({todos:
        this.state.todos.map((todo, index) =>{
          ...
          return index === pIndex ?{key, data:{name, isDone: !isDone }} : todo
        })
    })

  handleChange = taskName => this.setState({ topBar: taskName })

  handleOnClickDelete = pIndex => this.setState({ todos: this.state.todos.filter((todo, index) => index !== pIndex) })

  handleClearAllDone = () => this.setState({todos: this.state.todos.filter(({data:{isDone}})=>!isDone )})

  handleOption = pOption => this.setState({option: pOption})


  getDefaultStyles = () => this.state.todos.map(todo => ({...todo, style: {height: 0, opacity: 1}}))
  

  getListTodoAndStyle = () => {
    const { topBar, option, todos} = this.state;
    return todos.filter(({data:{name, isDone}}) => 
        ...
        <div className="app-container">
          ...
          <TransitionMotion
            ...
            {styles =>
              ...
              <Tasks todos={styles}
              onClickDelete={this.handleOnClickDelete} onClick={this.handleOnClick} />
            }
          </TransitionMotion>
